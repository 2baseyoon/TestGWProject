...
<div class="cRConTop titType">
	<h2 class="title"></h2>This is legacy page.
				...
				<div class="contbox" > <!-- 상단 영역 확장시 값 변경 (기본 125px;) -->
					...
					<div class="conin_view" id="conin_view"><!-- 좌우 폭 조정에 따라 값 변경(좌측 width값) -->
						...
						<div id="IframeDiv" style="display: none;">
							<iframe id="Iframe" name="Iframe" frameborder="0" width="100%" height="100%" class="wordLayout" scrolling="no"></iframe>
						</div>
			 ...
			 headerData =[	/// 결재 상세정보 표시 ////
			                  ...
			                  {key:'FormSubject', label:'<spring:message code="Cache.lbl_apv_subject"/>', width:'350', align:'left',			    // 제목
							   	  formatter:function () {		
							   			...
							   			if(CFN_GetCookie("ListViewCookie") == "Y"){
							   				html += "<a class=\"taTit\"" + ((this.item.ReadDate != "" && this.item.ReadDate != '0000-00-00 00:00:00')?"":" style=\"font-weight : 900;\"") + " onclick='onClickIframeButton(\""+this.item.ProcessID+"\",\""+this.item.WorkItemID+"\",\""+this.item.PerformerID+"\",\""+this.item.ProcessDescriptionID+"\",\""+this.item.FormSubKind+"\",\"\",\"\",\"\",\"\",\""+this.item.UserCode+"\",\""+this.item.BusinessData1+"\",\""+this.item.BusinessData2+"\",\""+this.item.TaskID+"\"); return false;'>"+this.item.FormSubject.toString().replaceAll("<", "&lt;").replaceAll(">", "&gt;")+"</a>";
							   			}else{
							   				html += "<a class=\"taTit\"" + ((this.item.ReadDate != "" && this.item.ReadDate != '0000-00-00 00:00:00')?"": " style=\"font-weight : 900;\"") + " onclick='onClickPopButton(\""+this.item.ProcessID+"\",\""+this.item.WorkItemID+"\",\""+this.item.PerformerID+"\",\""+this.item.ProcessDescriptionID+"\",\""+this.item.FormSubKind+"\",\"\",\""+this.item.FormInstID+"\",\"\",\"\",\""+this.item.UserCode+"\",\""+this.item.FormPrefix+"\",\""+this.item.BusinessData1+"\",\""+this.item.BusinessData2+"\",\""+this.item.TaskID+"\"); return false;'>"+this.item.FormSubject.toString().replaceAll("<", "&lt;").replaceAll(">", "&gt;")+"</a>";
							   			}
						   				...
						   				html += "</div><ui class=\"listinfo\"><li>"+ this.item.SubKind  +" </li><li>"+ this.item.InitiatorName + "</li><li>"+ this.item.InitiatorUnitName +"</li><li>"+ getStringDateToString("yyyy.MM.dd HH:mm:ss",this.item.Created)  +"</li><li>"+ CFN_GetDicInfo(this.item.FormName)  +"</li><li class=\"poRela\"><span class=\"usa\" style=\"display: none;\">유사양식</span></li></ul>";
			                	  ...
			                	  formatter:function () {
			                		  if(this.item.IsModify == "Y"){
			                			  if(this.item.ExtInfo.UseMultiEditYN == "Y") {
			                			 	 ...
			                			 	 return "<a onclick='openModifyPop(\""+this.item.ProcessID+"\",\""+this.item.WorkItemID+"\",\""+this.item.PerformerID+"\",\""+this.item.ProcessDescriptionID+"\",\""+this.item.FormSubKind+"\",\"\",\""+this.item.FormInstID+"\",\"\",\"\",\""+this.item.UserCode+"\",\""+this.item.FormPrefix+"\")'><img src=\"/approval/resources/images/Approval/ico_research_join.gif\" alt=\"\"></a>";
			                		 	 }
			                	 	 }
			                	  }
			                  }]; // 수정
			 ...
			 headerData =[	/// 결재 상세정보 표시하기 위해 추가함 ////
      					  {
			                  ...
			                  {key:'FormSubject', label:'<spring:message code="Cache.lbl_apv_subject"/>', width:'350', align:'left',						    // 제목
							   	  formatter:function () {
						   				...
						   				html += "</div><ui class=\"listinfo\"><li>"+ this.item.SubKind  +" </li><li>"+ this.item.InitiatorName  +"</li><li>"+ this.item.InitiatorUnitName +"</li><li>"+ getStringDateToString("yyyy.MM.dd HH:mm:ss",g_mode=="PreApproval"?this.item.Created:this.item.Finished) +"</li><li>"+ CFN_GetDicInfo(this.item.FormName)  +"</li><li class=\"poRela\"><span class=\"usa\" style=\"display: none;\">유사양식</span></li></ul>";		
		                  ...
		                  {key:'FormSubject', label:'<spring:message code="Cache.lbl_apv_subject"/>', width:'100', align:'left',						       // 제목
						   	  formatter:function () {
						   			...
						   			if(CFN_GetCookie("ListViewCookie") == "Y"){
						   				...
						   				html += "<a class=\"taTit\" onclick='onClickPopButton(\""+this.item.ProcessArchiveID+"\",\""+this.item.WorkitemArchiveID+"\",\""+this.item.PerformerID+"\",\""+this.item.ProcessDescriptionArchiveID+"\",\""+this.item.FormSubKind+"\",\"\",\""+this.item.FormInstID+"\",\"\",\"\",\""+this.item.UserCode+"\",\""+this.item.FormPrefix+"\",\""+this.item.BusinessData1+"\",\""+this.item.BusinessData2+"\",\""+this.item.TaskID+"\"); return false;'>"+this.item.FormSubject.toString().replaceAll("<", "&lt;").replaceAll(">", "&gt;")+"</a>";
						   			}
			 ...
			 headerData =[
		                  ...
		                  {key:'FormSubject', label:'<spring:message code="Cache.lbl_apv_subject"/>', width:'150', align:'left',						        // 제목
						   	  formatter:function () {
						   			...
						   			if(CFN_GetCookie("ListViewCookie") == "Y"){
						   				html += "<a class=\"taTit\" "+ ((this.item.ReadDate != "" && this.item.ReadDate != '0000-00-00 00:00:00')?"":" style=\"font-weight : 900;\"") + "onclick='onClickIframeButton(\""+this.item.ProcessID+"\",\"\",\"\",\"\",\"" + this.item.Kind + "\",\"\",\""+this.item.FormInstID+"\",\"\",\"\",\""+this.item.UserCode+"\",\""+this.item.BusinessData1+"\",\""+this.item.BusinessData2+"\",\""+this.item.TaskID+"\"); return false;'>"+this.item.FormSubject.toString().replaceAll("<", "&lt;").replaceAll(">", "&gt;")+"</a>";
						   			}else{
						   				html += "<a class=\"taTit\" "+ ((this.item.ReadDate != "" && this.item.ReadDate != '0000-00-00 00:00:00')?"":" style=\"font-weight : 900;\"") +"onclick='onClickPopButton(\""+this.item.ProcessID+"\",\"\",\"\",\"\",\""+this.item.Kind+"\",\"\",\""+this.item.FormInstID+"\",\"\",\"\",\""+this.item.UserCode+"\",\""+this.item.FormPrefix+"\",\""+this.item.BusinessData1+"\",\""+this.item.BusinessData2+"\",\""+this.item.TaskID+"\"); return false;'>"+this.item.FormSubject.toString().replaceAll("<", "&lt;").replaceAll(">", "&gt;")+"</a>";
						   			}
