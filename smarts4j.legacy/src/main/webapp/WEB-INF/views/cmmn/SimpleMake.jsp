	...
	<div class="middleCont">
		...
		<script>
			...
			function initSimpleMake(moduleName){
				if(moduleName == "Mail" && isLoad["Mail"] == false){
					...
					//CP 메일 사용할 경우에만 표시
					if(Common.getExtensionProperties("isUse.mail") == "Y"){
					    ...
					    success: function (res) {
					    	if(res.status == "SUCCESS"){
					    		...
					    		$(res.list).each(function(){
					    			...
					    			favoriteHtml += '<li><a onclick=\'CFN_OpenWindow("/approval/approval_Form.do?formID='+ this.FormID + '&mode=DRAFT", "", 790, (window.screen.height - 100), "resize", "false");\'>'+this.LabelText+'</a></li>';
					    		});
					...
					$.ajax({
					    ...
					    success: function (res) {
					    	if(res.status == "SUCCESS"){
					    		...
					    		$(res.list).each(function(){
					    			// TODO 가로 양식에 대한 처리 필요
					    			favoriteHtml += "<li><a onclick='onClickApprovalPopButton(\""+this.ProcessArchiveID+"\",\""+this.WorkitemArchiveID+"\",\""+this.PerformerID+"\",\""+this.ProcessDescriptionArchiveID+"\",\""+this.FormSubKind+"\",\"\",\"\",\"\",\"\",\""+this.UserCode+"\",\""+this.TYPE+"\",\""+this.FormPrefix+"\"); return false;'>"+this.FormSubject+"</a></li>";
					    		});
		...
		<div id="resourceSimpleMake" class="tabContentArrow">
			...
			<div class="bottomCont">
				...
				<a onclick="resourceUser.setOne('SC');" class="btnTypeDefault btnTypeBg ml5"><spring:message code='Cache.lbl_Reservation'/></a>
			</div>
