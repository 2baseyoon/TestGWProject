				...
				<ul>
					<c:forEach var="item" items="${result.list}">
						...
						<li class="listCol">
							...
							<div>
								<input type="text" class="inpFullSize" style="width:40%;" id="vacDay" value="${item.ExtVacDay}" maxlength="6" readonly disabled>
								vac&nbsp;
								<input type="text" class="inpFullSize" style="width:40%;" id="rewardVacDay" value="${item.RewardVacDay}" maxlength="6">
								min 
							</div>
						</li>
						<li class="listCol">
							...
							<div>
								<input type="text" class="inpFullSize" id="comment" value="" maxlength="500">
							</div>
						</li>
					</c:forEach>								
				</ul>							
...
<script>
	...
	$(document).ready(function() {
		...
		$("#rewardVacDay").on('change', function(e) {
			var sVacDay = parseInt($('#rewardVacDay').val() / 60) * 0.125;
			$('#vacDay').val(sVacDay);
		});
	...
	function updateVacDay() {
		...
		var params = {
			...
			vacDay : $('#vacDay').val(),
			rewardVacDay : $('#rewardVacDay').val(),
			oriExtVacDay: $('#oriExtVacDay').val(),
			comment : $('#comment').val(),
			registerCode : $('#registerCode').val()
		};
		
      	Common.Confirm("<spring:message code='Cache.lbl_vacationMsg70' />", "Confirmation Dialog", function (confirmResult) {
			if (confirmResult) {
		 		$.ajax({
					type : "POST",
					data : params,
					url : "/groupware/vacation/updateRewardVacDay.do",
