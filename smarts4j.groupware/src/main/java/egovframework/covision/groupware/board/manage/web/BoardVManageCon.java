...
public class BoardVManageCon {
	...
	public @ResponseBody CoviMap getFolderTreeList(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			...
			params.put("useDocMultiCategory", StringUtil.replaceNull(RedisDataUtil.getBaseConfig("useDocMultiCategory")));
	...
	public ModelAndView goFolderManagePopup(HttpServletRequest request, @RequestParam Map<String, String> paramMap) throws Exception{
		...
		CoviList AllBoardConfig = new CoviList();
		CoviList resultList = new CoviList();
		...
		AllBoardConfig = boardManageSvc.selectAllBoardConfig(params);
		
		for(Object AllBoardDefaultConfig : AllBoardConfig){
			CoviMap resultData = new CoviMap();
			resultData = (CoviMap) AllBoardDefaultConfig;
			if(resultData.getInt("LimitFileSize") > multipartResolver.getFileUpload().getSizeMax()) {
				resultData.put("LimitFileSize", multipartResolver.getFileUpload().getSizeMax());
			}
			
			if(resultData.getInt("TotalFileSize") > multipartResolver.getFileUpload().getSizeMax()) {
				resultData.put("TotalFileSize", multipartResolver.getFileUpload().getSizeMax());
			}
		...
		mav.addObject("allBoardConfig", resultList);
		mav.addObject("CommunityID", communityID);
		mav.addObject("domainID", domainID);
		mav.addObject("folderPathName", folderPathName);
		mav.addObject("isUseTransferFeed", RedisDataUtil.getBaseConfig("isUseTransferFeed", domainID));
	...
	public @ResponseBody CoviMap selectBoardACLData(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		try {
			...
			if("Root".equals(folderType)) {					//최상위 폴더의 상위권한상속을 체크 하였을 경우 메뉴 권한을 가져온다.
				params.put("objectID", menuID);				//현재 Folder가 참조하는 Menu ID
				params.put("objectType", "MN");				//Menu 권한 상속
				params.put("inheritedObjectID", menuID);	//상속 받는 ObjectID (MenuID)					
				result = authSvc.selectACL(params);			//Menu ACL 정보 조회
			} else {
				params.put("objectID", objectID);
				params.put("objectType", objectType);
				result = authSvc.selectACL(params);			//Folder ACL 정보 조회
			}
	...
	public @ResponseBody CoviMap createBoard(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		try {
			...
			if(folderType.equals("Root") || folderType.equals("DocRoot")){	//최상위 폴더 (다중분류 최상위) 의 sortPath, folderPath 
			...
			if(func.f_NullCheck(request.getParameter("communityID")).equals("")){
				...
				if("Root".equals(folderType) || "DocRoot".equals(folderType)) {					
				...
				insertACLData(aclArray, params, null);				//SYS_OBJECT_ACL
	...
	public @ResponseBody CoviMap updateBoard(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		try {
			...
			//확장옵션탭: 게시판설명 추가/삭제
			if((params.get("DescriptionBodyText") != null && !"".equals(params.get("DescriptionBodyText"))) 
					|| (params.get("DescriptionInlineImage") != null && !"".equals(params.get("DescriptionInlineImage")))){
				CoviMap editorparam = new CoviMap();
				
				editorparam.put("serviceType", "Board");
				editorparam.put("imgInfo", params.getString("DescriptionInlineImage"));
				editorparam.put("backgroundImage", params.getString("DescriptionBackgroundImage"));
				editorparam.put("objectID", params.getString("folderID"));
				editorparam.put("objectType","FD_EV");
				editorparam.put("messageID", "0");
				editorparam.put("frontStorageURL", params.getString("frontStorageURL"));
				editorparam.put("bodyHtml", params.getString("DescriptionBody"));
				
				CoviMap editorInfo = editorService.getEscapeContent(editorparam);
				
				params.put("bodySize", params.getString("DescriptionBodyText").length());
				params.put("boardDescription", editorInfo.getString("BodyHtml"));
				params.put("useBoardDescription", "Y");
			...
			if(cnt>0){
				...
				if("Y".equals(params.getString("aclIsChange"))){
					// 권한 변경 이력
					String logChangeID = LoggerHelper.authLogger(params, "Before");
					...
					//권한 등록
					insertACLData(aclArray, params, logChangeID);
					
					// 권한상속 - 기초설정 useBoardInherited 값이 Y 이고, 폴더/게시판 환경설정의 권상상속여부를 사용으로 저장하면 모든 하위의 권한이 같이 변경
					authSvc.setInheritedACL(
						params.getString("folderID"), "FD", 
						(RedisDataUtil.getBaseConfig("useBoardInherited", params.getString("domainID")).equals("Y") && "Y".equals(params.getString("isUpdateChildren")))
					);
...
//					targetParams.put("sortPath", pathData.get("SortPath") + String.format("%015d", Integer.parseInt(sortKey))+";");
	 ...
	 * @throws Exception
	 */
	public int insertACLData(CoviList aclArray, CoviMap params, String logChangeID) throws Exception{
		int cnt = 0;
	...
	public void insertInheritedACLData(CoviList aclArray, CoviMap params) throws Exception {
		...
		if (authList.size() > 0 && aclArray.size() > 0) {
			for (int i = 0; i < authList.size(); i++) {
				...
				insertACLData(aclArray, auth, null);
