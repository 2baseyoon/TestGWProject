		...
		<div class="eaccountingCont">
			<div class="eaccountingTopCont">
				...
				<div class="pagingType02 buttonStyleBoxLeft">
					...
					<!-- 전체 미상신 알림전송 -->
					<a class="btnTypeDefault" 			onclick="TaxInvoice.sendDelay(true);"><spring:message code="Cache.lbl_AccDelaySendAll"/></a>
					<!-- 선택 미상신 알림전송 -->
					<a class="btnTypeDefault" 			onclick="TaxInvoice.sendDelay(false);"><spring:message code="Cache.lbl_AccDelaySendSelect"/></a>
					<!-- 대상선택 미상신 알림전송 -->
					<a class="btnTypeDefault" 			onclick="TaxInvoice.OrgMap_Open();"><spring:message code="Cache.lbl_AccDelaySendSelectManager"/></a>
					<!-- 완료처리-->
...
(function(window) {
	var TaxInvoice = {
			...
			tossUserPopup_CallBack : function(info){
				...
				params.chkList		= chkList;
				params.tossUserCode	= info.tossSenderUserCode;	//전달받은사용자코드
				params.tossUserName	= info.tossSenderUserName;	//전달받은사용자이름
				params.tossComment	= info.tossComment;			//전달시코멘트
				...
				$.ajax({
					...
					success:function (data) {
						...
						if(data.status == "SUCCESS"){
							Common.Inform(data.tossUserName + "<spring:message code='Cache.ACC_msg_sendCompleteTaxInvoice' />"); //*** 사용자에게 수신내역을 전달했습니다.
							TaxInvoice.searchList();
						}else{
							Common.Error("<spring:message code='Cache.ACC_msg_error' />"); //오류가 발생했습니다. 관리자에게 문의바랍니다.
						}
			...
			OrgMap_Open : function(){
			    if(this.params.gridPanel.getCheckedList(0).filter(function(e) { return e.TaxInvoiceActiveCode == "N" }).length == 0){
					Common.Warning("<spring:message code='Cache.msg_noSelectDelayData' />"); //선택된 미상신내역이 없습니다.
					return;
			    }else{
					parent.Common.open("","orgmap_pop","<spring:message code='Cache.lbl_apv_org'/>","/covicore/control/goOrgChart.do?callBackFunc=goOrgChart_CallBack&type=B9","1060px","580px","iframe",true,null,null,true);
			    }
			},
			
			sendDelay: function(sendAll, arrData) {//미상신 알림메일
			    var me = this;
				var sendAlamList = [];
				var msg = "<spring:message code='Cache.lbl_AccSendNotiTaxAlert' />";
			  	//sendAll : true -> 전체 미상신 알림전송
				//			false -> 선택 미상신 알림전송
				if (!sendAll) {
					var checkList =  me.params.gridPanel.getCheckedList(0).filter(function(e) { return e.TaxInvoiceActiveCode == "N" });
					checkList.forEach(function(e) { sendAlamList.push(e.TaxInvoiceID); });
					...
					if(arrData != undefined && arrData.length > 0){
					    msg = "<spring:message code='Cache.lbl_AccSendNotiTaxAlert_2' />"; //선택한 세금계산서의 미상신건들에 대한 알림 메일을 세금계산서 담당자에게 전송합니다. 단, 담당자로 지정되지 않은 사람은 제외됩니다.
					}else{
					    msg = "<spring:message code='Cache.lbl_AccSendNotiTaxAlert_1' />"; //선택한 세금계산서의 미상신건들에 대한 알림 메일을 세금계산서 담당자에게 전송합니다.
					}
				}
				
				Common.Confirm(msg + "<br><spring:message code='Cache.msg_apv_ConfirmSend' /> ", "Confirmation Dialog", function (confirmResult) {
					if (confirmResult) {
						$.ajax({
							...
							data		:{
							    ...
							    "DelayManagerList": arrData.join(),
							    companyCode: accountCtrl.getComboInfo("companyCode").val()
							},
							success:function (data) {
								if(data.status == "SUCCESS"){
									Common.Inform("<spring:message code='Cache.msg_sms_send_success' /> ["+data.data.sendCnt+"<spring:message code='Cache.ACC_lbl_items' />]"
											+"\n["+data.data.sendDateRange+"]"); // 전송을 완료했습니다 [0건] [2023-03-01 ~ 2023-03-31]
								}else{
	...
	parent.goOrgChart_CallBack = setOrgMapData;
	function setOrgMapData(orgData){
		...
		if(arr.length != 0){
			TaxInvoice.sendDelay(false, arr);
		}else{
