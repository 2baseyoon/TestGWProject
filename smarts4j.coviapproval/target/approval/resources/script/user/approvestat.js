...
var ApvGraphicView = {
  ...
  getSubStep: function($$_operson, $$_otaskinfo){
    ...
    substep['name'] = CFN_GetDicInfo(me.getDisplayName(displayname));
    substep['dept'] = (oudisplayname) ? CFN_GetDicInfo(oudisplayname) : "";
    ...
    if(substep['comment_fileinfo'].length > 0){
    	...
    	if(stepUnitType == "ou"){
            substep['commentlink'] += "<div class=\"fClip\" style=\"float: none; display: inline-block; height: 13px;\"><a class=\"mClip\" onclick='openFileList_comment(this,"+(Number($$_operson.index().split("/")[0])+1).toString()+")'><spring:message code='Cache.lbl_attach'/></a></div>";
    	}else{
  ...
  // eval 함수 제거 및 array 직접 핸들링 231017 hgsong
