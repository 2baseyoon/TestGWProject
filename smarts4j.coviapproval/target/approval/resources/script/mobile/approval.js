...
$(document).on('pageinit', '#approval_view_page', function () {
	...
	//연결문서 상세보기(view), 편집 혹은 결재선(write)에서 뒤로가기시 현재 dialog 삭제하도록 함.
	if(($("div[id=approval_view_page]").length > 1 && mobile_comm_getQueryString("isDocLinked", $.mobile.activePage.attr("id")) == "Y") || $("div[id=approval_write_page]").length > 0){ 
		$("#"+$.mobile.activePage.attr("id")).remove();
	}
...
function mobile_approval_ListInit(){	
	...
	var isTeamsWrite = (typeof XFN_TeamsOpenGroupware == "function" && !teams_mobile && _mobile_approval_list.Mode == "Write") ? true : false;
...
// 팀즈 결재작성 목록 조회
function teams_approval_ChangeWriteMenu(mode) {
	if(!mode) mode = Write;
	...
	sTopMenu = mobile_comm_getDic("lbl_approval_writeApv");
	$('#approval_list_title').html("<span class=\"Tit\">" + sTopMenu + "</span>");
	...
	$.each($("#approval_list_content").children(), function(idx,ele) { ele.style.display = "none" });
	$("#approval_teams_list_formList").show();
	...
	$.ajax({
		...
		success: function (response) {
			if(response.status == "SUCCESS") {
				...
				//list show
				$.each($("#approval_list_content").children(), function(idx,ele) {
						if(ele.id == l_ActiveModule + "_list_div_Jobfunction" || ele.id == l_ActiveModule + "_list_div_BizDoc") {}  // 모바일 담당업무함,업무문서함 콤보박스 유지
	...
	for(var i = 0; i < objcnt; i++) {
		if($(obj).eq(i).is(":checked")) {
			...
			if(_mobile_approval_list.ListMode == "docSelect") $(".r_drop_menu").hide();
				...
				$(".r_drop_menu").show();
				$(".r_drop_menu").closest("div").removeClass("show");
			}
			cnt++;
		}
	}
...
function mobile_approval_clickwrite(pFormPrefix) {
	...
	if (isTeams) {
		...
		teams_approval_ChangeWriteMenu("Write");
...
function mobile_approval_clickmodify(open_mode) {
	...
	if(open_mode == "MODIFY") {
		...
		mobile_approval_popupMove(sUrl);
	} else {
		if (approvalViewQueryString && approvalViewQueryString.length > 0) {
			sUrl = "/approval/mobile/approval/write.do?" + approvalViewQueryString + "&editMode=N&isApvLineChg=N";
		} else {
...
//전자결재 편집/문서연결 dialog 이동
function mobile_approval_popupMove(sUrl){
	$("#"+$.mobile.activePage.attr( "id" )).remove();
	
	mobile_comm_showload();
	
	var sOption = { transition: 'pop', role: 'dialog', reload: true };
	sUrl += "&IsPopup=Y";
...
var ApvGraphicView = {
	...
	getSubStep: function($$_operson, $$_otaskinfo){
	  ...
	  substep['title'] = title == undefined ? "" : title;
	  substep['position'] = position == undefined ? "" : mobile_comm_getDicInfo(position.substring(position.indexOf(';')+1,position.length));
...
function mobile_approval_ViewInit(){
	...
	var returnVal = mobile_approval_formjsonload();	// formjson 데이터 parsing.
	if (returnVal == false) {
		mobile_comm_back();
		return false;
	}
	...
	if(parent.location.pathname) {
		if(parent.location.pathname.indexOf('portal') > -1 && parent.location.hash == '')
			_mobile_approval_view.isViewPanel = true;
	}
...
function mobile_approval_formload() {
    ...
    if (getInfo("Request.readtype") == 'preview') {
        ...
        // 미리보기 이미지 매핑
        if('EditorInlineImages' in formJson.FormInstanceInfo) {
        	var tbContentElement = formJson.BodyContext.tbContentElement;
        	var EditorInlineImages = [].concat(formJson.FormInstanceInfo.EditorInlineImages);
        	
        	for(var i = 0; i < EditorInlineImages.length; i++) {
        		var imgSrc = EditorInlineImages[i];
        		tbContentElement = tbContentElement.replace("‡"+i+"‡", "src=\""+imgSrc+"\" data-mce-src=\""+imgSrc+"\"");
        	}
        	
        	formJson.BodyContext.tbContentElement = tbContentElement;
        }
    }
...
function mobile_approval_formjsonload(bPassLoad) {
    ...
    if(!bPassLoad) mobile_comm_hideload();
}
...
function mobile_approval_openOrg(sMode) {
	...
	var	sMultiSelectYN = "N";
     ...
     case "ChangeReview": 
    	 ...
    	 sCallBackFn = "mobile_approval_setfoward('','"+sMode+"');";
    	 break;
     case "chargegroup": 
    	 sCallType = "Select"; 
    	 sCallBackFn = "mobile_approval_setfoward('','"+sMode+"');";
 ...
 * 전달시 선택한 부서/사용자 처리
 */
function mobile_approval_setfoward() {
	var pStrItemInfo = '{"item":[' + window.sessionStorage["userinfo"] + ']}';
	var oJSON = $.parseJSON(pStrItemInfo);
	
	// TODO
    var m_oChargeList = oJSON; // oList
    var elmRoot = $$(m_oChargeList);
    var elmlist = $$(m_oChargeList).find("item");
	var elmCnt = elmlist.concat().length;
    if (elmCnt == 0) {
        Common.Inform(mobile_comm_getDic("msg_apv_selectCharge")); // 결재권한자(개인/부서)를 선택해 주세요.
        return false;
    } else if (elmCnt > 1) {
    	Common.Inform(mobile_comm_getDic("msg_apv_selectChargeOne")); // 결재권한자(개인/부서) 항목은 1개만 선택해 주세요.
        return false;
    } else {
        try { document.getElementById("CHARGEID").value = $$(elmRoot).find("item").concat().eq(0).attr("AN"); } catch (e) { mobile_comm_log(e); }
        try { document.getElementById("CHARGENAME").value = $$(elmRoot).find("item").concat().eq(0).attr("DN"); } catch (e) { mobile_comm_log(e); }
        try { document.getElementById("CHARGEOUID").value = $$(elmRoot).find("item").concat().eq(0).attr("RG"); } catch (e) { mobile_comm_log(e); }
        
        var actionKind = "FORWARD";
...
function mobile_approval_getApprovalLine(apv_type) {
	...
	var tcinfoDel = false;
	var distributionDel = false;
	
	if(apv_type != undefined){
		if(apv_type == "tcinfoDel") tcinfoDel = true;
		if(apv_type == "distributionDel") distributionDel = true;
	}
	...
	if (getInfo("Request.mode") == "SUBREDRAFT" || getInfo("Request.mode") == "SUBAPPROVAL") {
        ...
        if ($$_m_oSubReStep.find("ou>person>taskinfo[kind='charge']").length == 0) {
			var tmpPerson = $$_m_oSubReStep.find("ou").find("person").json();
			tmpPerson.unshift({
			...
			$$_m_oSubReStep.find("ou").find("person").remove();
			$$_m_oSubReStep.find("ou").append('person',tmpPerson);
			...
			for(var j = 0; j < s_length; j++) {
				...
				if (getInfo("Request.mode") == "SUBREDRAFT" || getInfo("Request.mode") == "SUBAPPROVAL") {
					for(var k = 0 ; k < o_length ; k++) {
						...
						for(var p = 0 ; p < p_length ; p++) {
							...
							var position = (person.position && person.position.split(";").length > 1) ? mobile_comm_getDicInfo(person.position.split(";")[1], lang) : "";
				...
				// ou
					for(var k = 0 ; k < o_length ; k++) {
						...
						var position = "";
						if(type == "person" && temp.position && temp.position.split(";").length > 1){ 
							position = mobile_comm_getDicInfo(temp.position.split(";")[1], lang);
						}
						...
						if(step.unittype == "person") {
							...
							else if(step.routetype == "approve" && step.name == "reference") displayname = mobile_comm_getDic("lbl_apv_ref"); //참조자
							else if(step.routetype == "approve" && step.name == "confirm") displayname = mobile_comm_getDic("lbl_apv_PApprConfirm"); //확인자
						...
						// 삭제버튼 그리기
						if(isApvMod == "N" || temp.taskinfo.status == "pending" || (temp.taskinfo.kind == "charge" && !(step.routetype == "receive"))) {
	...
	// 참조
	if(getInfo("SchemaContext.scCC.isUse") == "Y" || tcinfoDel) {
		if(getInfo("Request.mode") !== "SUBREDRAFT" && getInfo("Request.mode") !== "SUBAPPROVAL" || tcinfoDel)
			$("#" + l_ActiveModule + "_write_tabtype").find("li").eq(1).show();
		else
			$("#" + l_ActiveModule + "_write_tabtype").find("li").eq(1).hide();
		if(apv_type == undefined || apv_type == "tcinfo" || apv_type == "tcinfoDel") {
			if(apvlist.steps.ccinfo != undefined) {
				...
				for(var i = 0; i < c_length; i++) {
					...
					if(getInfo("SchemaContext.scBeforCcinfo.isUse") == "Y" && (_mobile_approval_write.OpenMode == "DRAFT" || _mobile_approval_write.OpenMode == "APVLIST" || _mobile_approval_write.OpenMode == "TEMPSAVE")){
	...
	// 배포
	if(getInfo("SchemaContext.scIPub.isUse") == "Y" || distributionDel) {
		if(getInfo("Request.mode") !== "REDRAFT" && getInfo("Request.mode") !== "RECAPPROVAL" && getInfo("Request.mode") !== "SUBREDRAFT" && getInfo("Request.mode") !== "SUBAPPROVAL"  || distributionDel)
			$("#" + l_ActiveModule + "_write_tabtype").find("li").eq(2).show();
		else
			$("#" + l_ActiveModule + "_write_tabtype").find("li").eq(2).hide();
		if(apv_type == undefined || apv_type == "distribution"  || apv_type == "distributionDel") {
			if($("#ReceiveNames").val() != "") {
...
function mobile_approval_getApvTypeSelectOption(division, unittype, routetype, allottype, kind, name) {
	...
	if(unittype == "person" && !(routetype == "approve" && kind == "charge" && name == "기안자")) {
		if (routetype == "approve" && kind == "normal" && name == "일반결재") {
			...
			sHtml += "<option unittype=\"person\" routetype=\"approve\" allottype=\"\" kind=\"normal\" name=\"" + mobile_comm_getDic("lbl_apv_normalapprove") + "\">" + mobile_comm_getDic("lbl_apv_normalapprove") + "</option>"; //일반결재, 일반결재
		}
		...
		if(getInfo("Request.mode") != "SUBREDRAFT" && getInfo("Request.mode") != "SUBAPPROVAL" && division == "0") {
		    ...
		    if (getInfo("SchemaContext.scPAgr.isUse") == "Y"){
				if(routetype == "consult" && allottype == "parallel") {
			    	sHtml += "<option unittype=\"person\" routetype=\"consult\" allottype=\"parallel\" kind=\"consult\" name=\"" + mobile_comm_getDic("lbl_apv_Par") + "\" selected>" + mobile_comm_getDic("lbl_apv_PersonConsent") + "</option>"; //병렬합의, 개인합의(병렬)
				}else {
			    	sHtml += "<option unittype=\"person\" routetype=\"consult\" allottype=\"parallel\" kind=\"consult\" name=\"" + mobile_comm_getDic("lbl_apv_Par") + "\">" + mobile_comm_getDic("lbl_apv_PersonConsent") + "</option>"; //병렬합의, 개인합의(병렬)
				}		
			} 
		    ...
		    if (getInfo("SchemaContext.scPCooPL.isUse") == "Y"){
		    	if(routetype == "assist" && allottype == "parallel"){
			    	sHtml += "<option unittype=\"person\" routetype=\"assist\" allottype=\"parallel\" kind=\"normal\" name=\"" + mobile_comm_getDic("lbl_apv_assist_2") + "\" selected>" + mobile_comm_getDic("lbl_apv_assist_2") + "</option>"; //개인협조(병렬), 개인협조(병렬)
				} else{
		    		sHtml += "<option unittype=\"person\" routetype=\"assist\" allottype=\"parallel\" kind=\"normal\" name=\"" + mobile_comm_getDic("lbl_apv_assist_2") + "\">" + mobile_comm_getDic("lbl_apv_assist_2") + "</option>"; //개인협조(병렬), 개인협조(병렬)
				}
		    }
		    
		    if (getInfo("SchemaContext.scPCoo.isUse") == "Y") {
		    	if (routetype == "assist" && allottype == "serial") {
		    		sHtml += "<option unittype=\"person\" routetype=\"assist\" allottype=\"serial\" kind=\"normal\" name=\"" + mobile_comm_getDic("lbl_apv_assist") + "\" selected>" + mobile_comm_getDic("lbl_apv_assist") + "</option>"; //개인협조, 개인협조
		    	} else {
		    		sHtml += "<option unittype=\"person\" routetype=\"assist\" allottype=\"serial\" kind=\"normal\" name=\"" + mobile_comm_getDic("lbl_apv_assist") + "\">" + mobile_comm_getDic("lbl_apv_assist") + "</option>"; //개인협조, 개인협조
		    	}
		    }
		}
	    /*
		if (getInfo("SchemaContext.scPRec.isUse") == "Y")
	    	sHtml += "<option unittype=\"person\" routetype=\"receive\" allottype=\"\" kind=\"charge\">" + mobile_comm_getDic("lbl_apv_charge_approve") + "</option>"; //divisiontype = "receive" //담당결재
		*/
	    if (getInfo("SchemaContext.scPConfirm.isUse") == "Y"){
	    	var confirmSelected = kind=="confirm" ? "selected" : "";
	    	sHtml += "<option unittype=\"person\" routetype=\"approve\" allottype=\"\" kind=\"confirm\" name=\"confirm\" "+confirmSelected+">" + mobile_comm_getDic("lbl_apv_PApprConfirm") + "</option>"; //확인자
	    }
	    if (getInfo("SchemaContext.scPShare.isUse") == "Y"){
	    	var referenceSelected = kind=="reference" ? "selected" : "";	    	
	    	sHtml += "<option unittype=\"person\" routetype=\"approve\" allottype=\"\" kind=\"reference\" name=\"reference\" "+referenceSelected+">" + mobile_comm_getDic("lbl_apv_ref") + "</option>"; //참조자
	    }
	} else if(unittype == "ou"){
		if(division == "0") {
		    ...
		    if (getInfo("SchemaContext.scDAgr.isUse") == "Y"){
		    	if(routetype == "consult" && allottype == "parallel"){
			    	sHtml += "<option unittype=\"ou\" routetype=\"consult\" allottype=\"parallel\" kind=\"consult\" name=\"" + mobile_comm_getDic("lbl_apv_DeptConsent_2") + "\" selected>" + mobile_comm_getDic("lbl_apv_DeptConsent_2") + "</option>"; //부서합의(병렬), 부서합의(병렬)
				} else {
			    	sHtml += "<option unittype=\"ou\" routetype=\"consult\" allottype=\"parallel\" kind=\"consult\" name=\"" + mobile_comm_getDic("lbl_apv_DeptConsent_2") + "\">" + mobile_comm_getDic("lbl_apv_DeptConsent_2") + "</option>"; //부서합의(병렬), 부서합의(병렬)
				}
		    } 
		    ...
		    if (getInfo("SchemaContext.scDCooPL.isUse") == "Y"){
		    	if(routetype == "assist" && allottype == "parallel"){
			    	sHtml += "<option unittype=\"ou\" routetype=\"assist\" allottype=\"parallel\" kind=\"normal\" name=\"" + mobile_comm_getDic("lbl_apv_DeptAssist_2") + "\" selected>" + mobile_comm_getDic("lbl_apv_DeptAssist_2") + "</option>"; //부서협조(병렬), 부서협조(병렬)
			
				} else {
			    	sHtml += "<option unittype=\"ou\" routetype=\"assist\" allottype=\"parallel\" kind=\"normal\" name=\"" + mobile_comm_getDic("lbl_apv_DeptAssist_2") + "\">" + mobile_comm_getDic("lbl_apv_DeptAssist_2") + "</option>"; //부서협조(병렬), 부서협조(병렬)
				}
			}
		    
		    if (getInfo("SchemaContext.scDCoo.isUse") == "Y" && !(routetype == "assist" && allottype == "serial")) {
		    	if (routetype == "assist" && allottype == "serial") {
		    		sHtml += "<option unittype=\"ou\" routetype=\"assist\" allottype=\"serial\" kind=\"normal\" name=\"" + mobile_comm_getDic("lbl_apv_DeptAssist") + "\" selected>" + mobile_comm_getDic("lbl_apv_DeptAssist") + "</option>"; //부서협조, 부서협조
		    	} else {
		    		sHtml += "<option unittype=\"ou\" routetype=\"assist\" allottype=\"serial\" kind=\"normal\" name=\"" + mobile_comm_getDic("lbl_apv_DeptAssist") + "\">" + mobile_comm_getDic("lbl_apv_DeptAssist") + "</option>"; //부서협조, 부서협조
		    	}
		    }
		    	
		}
	    if (getInfo("SchemaContext.scDRec.isUse") == "Y" || getInfo("SchemaContext.scDRecEnt.isUse") == "Y" || getInfo("SchemaContext.scDRecReg.isUse") == "Y")
	    	sHtml += "<option unittype=\"ou\" routetype=\"receive\" allottype=\"\" kind=\"\">" + mobile_comm_getDic("lbl_apv_DeptAssist") + "</option>"; //수신처
	}
...
function mobile_approval_moveApvUser(pMode, obj) {
	...
	var o_num = $(obj).parents("li").attr("id").replace("approval_write_ApvList", "").split("_")[2];
	var apvlist = JSON.parse($("#APVLIST").val());
	var last1 = 0;
	var last2 = 0;
	var division1 = null;
	...
	if(getInfo("Request.mode") == "SUBREDRAFT" || getInfo("Request.mode") == "SUBAPPROVAL") {
		...
		if(s_num > 0) {
			...
			o_num = $$(division1).find("step > ou").has("taskinfo[status='pending'][piid='" + getInfo("ProcessInfo.ProcessID").toUpperCase() + "']").index(); //ou
...
// 결재선 - 결재선 추가
function mobile_approval_addApprovalLine(pMode, addApvType) {
	_mobile_approval_write.ApvLineMode = pMode;
	
	var sUrl = "/covicore/mobile/org/list.do";
	
	// 기안부서 내 결재선 편집하는 경우 이외에 사용자에 한하여 추가 가능
	if(getInfo("Request.mode") == "DRAFT" || getInfo("Request.mode") == "TEMPSAVE" || getInfo("Request.mode") == "APPROVAL") {
		// [2020-02-03 MOD] 병렬 관련 옵션 미사용 처리(추후 보완 예정)
		
		// [부서 추가] 결재 : 부서준법, 부서감사, 부서합의(병렬), 부서합의(순차), 부서협조(병렬), 부서협조(순차) / 참조 / 배포
		var appSchemaOptionArr = [getInfo("SchemaContext.scDAdt1.isUse"), getInfo("SchemaContext.scDAdt.isUse"), getInfo("SchemaContext.scDAgr.isUse"), getInfo("SchemaContext.scDAgrSEQ.isUse"), getInfo("SchemaContext.scDCooPL.isUse"), getInfo("SchemaContext.scDCoo.isUse")];
		if(	(pMode == "approval" && (appSchemaOptionArr.includes('Y'))) ||
			(pMode == "tcinfo" && (getInfo('SchemaContext.scCC.value').match(/Y$/gi))) ||
			(pMode == "distribution" && (getInfo('SchemaContext.scIPub.isUse') == "Y")) ) {
...
function mobile_approval_setApprovalLine() {
	var l_ActiveModule = mobile_approval_getActiveModule();
	var l_ActivePageObj = window["_mobile_" + l_ActiveModule + "_write"];
	var addApvType = this.addApvType; //addApvType != undefined ? division 신규 추가
	...
	if(l_ActivePageObj.ApvLineMode == "approval") {
		$(userinfos).each(function(i, userinfo) {
			...
			if(addApvType == undefined) {
				...
				//수신자가 지정되어 있지 않은 경우를 위해 추가
				apvlist.steps.division = [].concat(apvlist.steps.division);
				
				//division
				if(apvlist.steps.division.length == undefined || apvlist.steps.division.length == 0) {
					apvlist.steps.division.concat(new Object);
				} else {
					apvlist.steps.division.push(new Object);
				}
				division = apvlist.steps.division[apvlist.steps.division.length-1];
				
				//new_step
				new_step = new Object;
				new_step.ou = new Object;
				new_step.ou.person = new Object;
				new_step.ou.person.taskinfo = new Object;
			}
			...
			if(userinfo.itemType == "user") {
				...
				if(getInfo("SchemaContext.scChkDuplicateApv.isUse") == "Y") {
					//결재자 중복체크 (발신처/수신처)
					if(mobile_approval_chkDupApprovalLine(userinfo.AN)) {
			    		return true;
					}
				}
				...
				if(addApvType != undefined) {
					//division, step 공통 데이터 보정
					var divisionTaskinfo = {};
					var displayName = "";
					
					if(addApvType == "receive") displayname = mobile_comm_getDic("lbl_apv_charge_approve"); 	//담당결재
					
					new_step.name = displayname;
					new_step.unittype = "person";
					new_step.routetype = addApvType;
					
					new_step.ou.name = userinfo.RGNM;
					
					new_step.ou.person.name = userinfo.DN;
					new_step.ou.person.ouname = userinfo.RGNM;
					
					var position = userinfo.PO.replaceAll("|", ";");
					var title = userinfo.TL.replaceAll("|", ";");
					var level = userinfo.LV.replaceAll("|", ";");
					
					new_step.ou.person.position = position;
					new_step.ou.person.title = title;
					new_step.ou.person.level = level;
					
					division.divisiontype = addApvType;
					division.name = displayname;
					division.oucode = new_step.ou.person.oucode;
					division.ouname = new_step.ou.person.ouname;
					
					divisionTaskinfo.result = "inactive";
					divisionTaskinfo.kind = addApvType;
					divisionTaskinfo.status = "inactive";
					
					division.taskinfo = divisionTaskinfo;
					
					// new step 수정
					if(addApvType == "receive") { // 담당결재(담당자)
						new_step.name = displayname;
						new_step.unittype = "person";
						new_step.routetype = "receive";
						
						new_step.ou.person.taskinfo.status = "inactive";
						new_step.ou.person.taskinfo.result = "inactive";
						new_step.ou.person.taskinfo.kind = "charge";
					}
				}
			...
			if(division.step == undefined) {
				delete division.step;
				division.step = new_step;
			} else if(division.step.length == undefined) {
...
function mobile_approval_delApvUser(obj) {
	...
	//참조
		...
		mobile_approval_getApprovalLine("tcinfoDel");	
	}
	//배포
		...
		mobile_approval_getApprovalLine("distributionDel");
	}
}
...
function mobile_approval_changeApvSelect(obj) {
	...
	if(d_num == "0") {		
		...
		} else if(step.allottype) {
			delete step.allottype;
		}
		if(unittype == "person") {
...
function mobile_approval_draft() {
	...
	else if(mobile_approval_draftvalidation()) { //기안 전 결재선 validation 체크
		return;
	}
	else {
...
function mobile_approval_draftvalidation() {	
	//결재선 중복체크
	if(getInfo("SchemaContext.scChkDuplicateApv.isUse") == "Y") {
		if(mobile_approval_chkDupApprovalLine()) {
	        return true;
		}
	}
...
function mobile_approval_chkDupApprovalLine(userCode) {
    var dupCnt = 0;
    var msg = "";
	var apvlist = JSON.parse($("#APVLIST").val());
    ...
    // 중복 결재자 수 (기안 전 결재선 체크 등)
    if(!userCode) {
    	dupCnt = arrPersonCode.filter((code,idx) => arrPersonCode.indexOf(code) !== idx).length;
    	msg = mobile_comm_getDic("msg_apv_519"); //중복된 결재자가 있습니다.
    }
    // 특정 사용자 중복 확인 (결재자 추가 등)
    else if(userCode) {
    	dupCnt = arrPersonCode.filter(code => code == userCode).length;
    	msg = mobile_comm_getDic("msg_apv_186"); //결재자가 이미 지정되어 있습니다.
    }
...
function mobile_approval_write_clickTab(obj, target, isPageLoad) {
	...
	//if((value != "approval_write_div_formSelect" && _mobile_approval_write.FormID != "") || value == "approval_write_div_formSelect") {
		...
		if(value == "approval_write_div_detailItem" || value == "approval_write_div_approvalLine" || value == "approval_write_div_preview") {
			if($("#APVLIST").val() != "" && value == "approval_write_div_approvalLine") {
				mobile_approval_formjsonload(true);
				mobile_approval_getApprovalLine();
			}else if(typeof(getInfo) == 'undefined' || getInfo("FormInfo.FormID") != _mobile_approval_write.FormID || isLoad == "N") {
				mobile_approval_getFormInfo();
			}
			...
			if(getInfo("SchemaContext.scPRec.isUse") == "Y" || getInfo("SchemaContext.scPRecEnt.isUse") == "Y" || getInfo("SchemaContext.scPRecReg.isUse") == "Y") {
				$("#approval_write_div_approval").find("a[name=addReceiveApv]").show();
			} else {
				$("#approval_write_div_approval").find("a[name=addReceiveApv]").hide();
			}
...
function mobile_approval_setDocClassList() {
   ...
   if(isEdmsPopup == true){
		$.ajax({
			...
			data:{
				"leftMenuType": "Total",
				"menuID": mobile_comm_getBaseConfig("DocMenu"),
				"bizSection": "Doc",
				"isEDMS": "Y",
				"docMultiFolder": "N"
			},
...
function mobile_approval_deleteCheck(type){
	...
	for(var i = 0; i < checkcnt; i++) {
		if($(checked[i]).val() != "") 
			checkCheckList[i] = JSON.parse($(checked[i]).val());
	}
...
//포탈 미결문서 분할보기 - 승인/반려 후 목록의 첫 번째 양식 클릭
function mobile_approval_ClickFirstList() {
	if($(".portal_approval .post_list>ul>li").length == 0) {
		var emptyHtml = "<div class='ui_empty'><div class='message'><p>"+Common.getDic('msg_clickListForShow')+"</p></div></div>"; //좌측 목록을 클릭하시면 내용 미리보기가 가능합니다.
		$(".portal_approval .view_panel").html(emptyHtml);
	} else {
		$(".portal_approval .post_list>ul>li>a").get(0).click();
		mobile_approval_disableBtns(true, true);
	}
}
