...
function initButtons() {
	if(m_sApvMode != "CFADMIN") {
		...
		// 전결규정에서 변경한 담당업무 정보
		//chargeJob = $("#RuleChargeJob", m_oFormMenu.document).length > 0 ? $("#RuleChargeJob", m_oFormMenu.document).val() : "";
		if(m_oFormMenu.getChargeJob) chargeJob = m_oFormMenu.getChargeJob();
    ...
    if ((getInfo("SchemaContext.scCC.isUse") != "Y"  && getInfo("SchemaContext.scIPub.isUse") != "Y") || m_sApvMode == "CFADMIN") {
        ...
        if (getInfo("Request.mode") == "REDRAFT" || getInfo("Request.mode") == "RECAPPROVAL" || getInfo("mode") == "SUBREDRAFT") {
        	...
        	if(getInfo("SchemaContext.scGRec.isUse") != "N"){ // 배포목록 활성화
        ...
        switch (m_sApvMode) {
            case "APPROVAL":
            	if($$_m_oApvList.find("division>step").concat().has("taskinfo[status=pending],[status=reserved]").parent().index() == 0){
	                ...
	                if (getInfo("SchemaContext.scPRec.isUse") == "Y" || getInfo("SchemaContext.scPRecEnt.isUse") == "Y" || getInfo("SchemaContext.scPRecReg.isUse") == "Y") $("#btCharge").attr("style", "display:;");
	                ...
	                if (getInfo("SchemaContext.scDRec.isUse") == "Y" || getInfo("SchemaContext.scDRecEnt.isUse") == "Y" || getInfo("SchemaContext.scDRecReg.isUse") == "Y") $("#btReceipt").attr("style", "display:;");
    ...
    for(var i=nrowTBindex-1; i>=0; i--){
    	if($(oTBody.rows[i]).children().eq(division_divisiontype).text() != "receive"){
	        ...
	        if (tmpIndex.length == 1) {
				...
				if($(oTBody.rows[i]).context.innerHTML.includes("parallel") && $(oTBody.rows[i+1]).context.innerHTML.includes("parallel")){
					tmpIndex = dblDigit(Number(tmpIndex));
				}else{
	    	...
	    	//담당자 추가할 경우 순번 발번을 위한 배열 생성
			var receiveArr = [];
			for(var j=i; j>=0; j--){
				if($(oTBody.rows[j]).context.innerHTML.includes("receive")){ 
					receiveArr.push(j); 
				}
			}
			
	    	//병렬 순번 발번을 위한 조건 추가
			if($(oTBody.rows[i]).context.innerHTML.includes("parallel") && !($(oTBody).context.innerHTML.includes("receive"))) {
				for(var j=i; j>=0; j--){
					$(oTBody.rows[j].childNodes[dp_apv_no]).html(getDotCountSpace(tmpIndex));
				}
			}else if($(oTBody.rows[i]).context.innerHTML.includes("parallel") && ($(oTBody).context.innerHTML.includes("receive"))) {
				for(var j=i; j>receiveArr[0]; j--){
					$(oTBody.rows[j].childNodes[dp_apv_no]).html(getDotCountSpace(tmpIndex));
				}
			}else{
...
function deleteCC(obj) {
    ...
    if(oTable.querySelector("tbody").innerText == ''){
		oTable.querySelector("tbody").innerHTML = "<tr id='hTRTemp' data-type='temp' ondrop=\"void_drop(event,'tblccinfo')\" ondragover=\"event.preventDefault();\"><td colspan='3'></td></tr>"
	}
}
...
function insertToList(oList) {
    ...
    if (m_sSelectedUnitType != "ou" && m_sApvMode != "SUBREDRAFT" && getInfo("SchemaContext.scChkDuplicateApv.isUse") == "Y") {
        ...
        $$_oSrcDoc = chkDuplicateApprover($$_oSrcDoc);
    } else if (m_sSelectedUnitType == "ou" && (m_sSelectedRouteType == "assist" || m_sSelectedRouteType == "consult")) {
        $$_oSrcDoc = chkDuplicateApprover($$_oSrcDoc);
    }
    ...
    $$_oSrcDoc.find("item").concat().each(function (i, $$_element) {
        ...
        if (m_sSelectedUnitType == "ou") {
            ...
            arrRolePerson.push(unescapeHtml($$_element.attr("DN"))); //roleperson_name
            ...
            arrRolePerson.push(unescapeHtml($$_element.attr("SGNM")))//roleperson_ouname
        	...
        	//if($$_ostep.find("person>taskinfo").attr("visible") != "n" && $$_ostep.find("person>taskinfo").attr("kind") != "bypass"){
	            ...
	            if (steproutetype == "notify") {
	                ...
	                $$_ostep.find("ou").concat().each(function (index_ou, $$_oou) {
                        ...
                        var pfid = "";
                        var widescid = "";
                        taskid = $$_oou.attr("taskid");
                        wiid = $$_oou.attr("wiid");
                        pfid = $$_oou.attr("pfid");
                        widescid = $$_oou.attr("widescid");
	                    ...
	                    $$_oou.find("person", "role").concat().each(function (index_person, $$_operson) {
	                        ...
	                        if ($$_operson.nodename() == "role") {
	                            ...
	                            oudisplayname = $$_operson.attr("ouname");
	                            code = $$_operson.attr("code");
	                            taskid = $$_operson.attr("taskid");
	                            wiid = $$_operson.attr("wiid");
	                            pfid = $$_operson.attr("pfid");
	                            widescid = $$_operson.attr("widescid");
	                        }
...
function refreshCC(bAll) {
    ...
    //참조자 Drag&Drop 가능하도록 수정
    if(document.getElementById("tblccinfo").querySelector("tbody").innerText == ''){
			document.getElementById("tblccinfo").querySelector("tbody").innerHTML =
				"<tr id='hTRTemp' data-type='temp' ondrop=\"void_drop(event,'tblccinfo')\" ondragover=\"event.preventDefault();\"><td colspan='3'></td></tr>";
	}
}
...
--====================================*/
var chargeJob = ""; // 전결규정에서 변경한 담당업무 정보
...
function setRuleApvLine(isChanged, ruleItem) {
	...
	$(".nea_appline_bot").find("ul").append(
		$step.map( function (idx, item) {
			var ous = item.ou;
			if(item.length > 0) { ous = item.map(obj => obj.ou); }
			var $li = 
				ous.length > 1
					? $(ous).map( function(i, ou) { return $("<li>").html(ou.person ? CFN_GetDicInfo(ou.person.name) : CFN_GetDicInfo(ou.name)); } )
					: $("<li>").html(ous.person ? CFN_GetDicInfo(ous.person.name) : CFN_GetDicInfo(ous.name));				
			return $li;
...
function setApvList() {
	if (evaluateApvList()) {
        ...
        //빈칸인 참조자 라인 삭제
		if($$_m_oApvList.find("steps>ccinfo").concat().length == 1 && $$_m_oApvList.find("steps>ccinfo").concat().eq(0).attr("senderid") == ""){
			$$_m_oApvList.find("steps>ccinfo").remove();
		}
      ...
      //대외수신처 관련 종료
        ...
        var apvlist = true; //결재선 팝업창 확인 후 의견 조회 안되게 수정
        var oApvList;
        if (admintype == "ADMIN") {//관리자 일 경우 결재선 변경 함수 호출
            ...
            if (bChange) {
                var ApvLines = m_oFormEditor.__setInlineApvList(oApvList, apvlist);
			...
			// 전결규정에서 변경한 담당업무 정보
			//if($("#RuleChargeJob", m_oFormMenu.document).length > 0) $("#RuleChargeJob", m_oFormMenu.document).val(chargeJob);
			if(m_oFormMenu.setChargeJob) m_oFormMenu.setChargeJob(chargeJob);
...
function doButtonAction(obj) {
    ...
    switch (obj.id) {
		...
		case "btPerson": // 일반결재
		    ...
		    if (m_sApvMode == "CHARGE") {
		        addChargePerson();
		    } else if (m_sApvMode == "REDRAFT" || m_sApvMode == "RECAPPROVAL" || m_sApvMode == "DEPTLIST") { // 20230209 : 부서수신에서 담당자 지정시 지정된 사용자 미결함에서 "RECAPPROVAL" 모드로 열리는 현상으로 결재선 추가 할 때 꼬이는 현상이 있어 "RECAPPROVAL" 도 추가함 // 20230623 : 일괄결재선에서 결재자 추가시 꼬이는 현상으로 "DEPTLIST" 추가
		        addReceiptPerson();
		    } else {
		        addPerson();
		    }
...
//기존 widescid,pfid,taskid,wiid 추가하는 부분 주석
...
//    });
    ...
    for (var y = 0; y < oTBody.rows.length ; y++) {
		if(oTBody.rows[y].id == "hTRTemp"){ // D&D 를 위한 영역 Skip.
			continue;
		}
...
function makeApvLineDivisionXml(oTr) {
    ...
    if ($(oTr.childNodes[division_processID]).text() != "") $$_oDivision.attr("processID", $(oTr.childNodes[division_processID]).text());
    if ($(oTr.childNodes[division_processDescID]).text() != "") $$_oDivision.attr("processDescID", $(oTr.childNodes[division_processDescID]).text());
...
function makeApvLineOuXml(oTr, $$_oStep) {
    ...
    if ($(oTr.childNodes[ou_taskid]).text() != "") $$_oOu.attr("taskid", $(oTr.childNodes[ou_taskid]).text());
    if ($(oTr.childNodes[ou_wiid]).text() != "") $$_oOu.attr("wiid", $(oTr.childNodes[ou_wiid]).text());
    if ($(oTr.childNodes[ou_pfid]).text() != "") $$_oOu.attr("pfid", $(oTr.childNodes[ou_pfid]).text());
    if ($(oTr.childNodes[ou_widescid]).text() != "") $$_oOu.attr("widescid", $(oTr.childNodes[ou_widescid]).text());
...
function makeApvLineRolePersonXml(oTr, $$_oOu) {
    ...
    if ($(oTr.childNodes[roleperson_person_taskid]).text() != "") $$_oRolePerson.attr("taskid", $(oTr.childNodes[roleperson_person_taskid]).text());
    if ($(oTr.childNodes[roleperson_person_wiid]).text() != "") $$_oRolePerson.attr("wiid", $(oTr.childNodes[roleperson_person_wiid]).text());
    if ($(oTr.childNodes[roleperson_person_pfid]).text() != "") $$_oRolePerson.attr("pfid", $(oTr.childNodes[roleperson_person_pfid]).text());
    if ($(oTr.childNodes[roleperson_person_widescid]).text() != "") $$_oRolePerson.attr("widescid", $(oTr.childNodes[roleperson_person_widescid]).text());
...
function chkDuplicateApprover(oSrcDoc) {
    ...
    oSrcDocList.each(function(i, item){
        ...
        case "person":
            if (m_sSelectedStepType == "ccinfo") {
                ...
                $("#tblApvLine").find("tr").each(function (i, itemTR) {
                	var itemKind = $(itemTR.childNodes[roleperson_taskinfo_kind]).text();
                	if(itemKind != "conveyance" && itemKind != "substitute") {
        ...
        case "ou":
            ...
            } else if (m_sSelectedRouteType == "assist") {
                $("#tblApvLine").find("tr").each(function (i, itemTR) {
                    if ($(itemTR.childNodes[ou_code]).text() == $$(item).attr("AN") && $(itemTR.childNodes[step_unittype]).text() == "ou" && $(itemTR.childNodes[step_routetype]).text() == "assist") {
                        try {
                        	$$(item).attr("isDeleted", "Y");
                            bDeleteAS = true;
                        } catch (e) {
            ...
            } else if (m_sSelectedRouteType == "consult") {
                $("#tblApvLine").find("tr").each(function (i, itemTR) {
                    if ($(itemTR.childNodes[ou_code]).text() == $$(item).attr("AN") && $(itemTR.childNodes[step_unittype]).text() == "ou" && $(itemTR.childNodes[step_routetype]).text() == "consult") {
                        try {
                        	$$(item).attr("isDeleted", "Y");
                            bDeleteCO = true;
                        } catch (e) {
    ...
    if (bDeleteAS) { Common.Warning(Common.getDic("msg_apv_dupChkAssist")); } //동일부서를 협조로 지정할 수 없습니다.
    if (bDeleteCO) { Common.Warning(Common.getDic("msg_apv_dupChkConsult")); } //동일부서를 합의로 지정할 수 없습니다.
    return oSrcDoc;
...
//itemtaskinfo: $$jsoner
    ...
    if ($$_nDivision != undefined && $$_nDivision.exist()) {
        ...
        arrDivision.push(($$_nDivision.attr("ouname") == null) ? "" : unescapeHtml($$_nDivision.attr("ouname")));
        arrDivision.push(($$_nDivision.attr("processID") == null) ? "" : $$_nDivision.attr("processID"));
        arrDivision.push(($$_nDivision.attr("processDescID") == null) ? "" : $$_nDivision.attr("processDescID"));
    }
    ...
    if ($$_nOu != null) {
        ...
        arrOu.push(($$_nOu.attr("name") == null) ? "" : unescapeHtml($$_nOu.attr("name")));
        arrOu.push(($$_nOu.attr("taskid") == null) ? "" : $$_nOu.attr("taskid"));
        arrOu.push(($$_nOu.attr("wiid") == null) ? "" : $$_nOu.attr("wiid"));
        arrOu.push(($$_nOu.attr("pfid") == null) ? "" : $$_nOu.attr("pfid"));
        arrOu.push(($$_nOu.attr("widescid") == null) ? "" : $$_nOu.attr("widescid"));
    }
    ...
    if ($$_nRolePerson != null) {
        ...
        arrRolePerson.push(($$_nRolePerson.attr("name") == null) ? "" : unescapeHtml($$_nRolePerson.attr("name")));
        ...
        arrRolePerson.push(($$_nRolePerson.attr("ouname") == null) ? "" : unescapeHtml($$_nRolePerson.attr("ouname")));
        arrRolePerson.push(($$_nRolePerson.attr("sipaddress") == null) ? "" : $$_nRolePerson.attr("sipaddress"));
        arrRolePerson.push(($$_nRolePerson.attr("taskid") == null) ? "" : $$_nRolePerson.attr("taskid"));
        arrRolePerson.push(($$_nRolePerson.attr("wiid") == null) ? "" : $$_nRolePerson.attr("wiid"));
        arrRolePerson.push(($$_nRolePerson.attr("pfid") == null) ? "" : $$_nRolePerson.attr("pfid"));
        arrRolePerson.push(($$_nRolePerson.attr("widescid") == null) ? "" : $$_nRolePerson.attr("widescid"));
    }
    ...
    if ($$_nRolePerson_Person != null) {
    	...
    	arrRolePerson_person.push(($$_nRolePerson_Person.attr("name") == null) ? "" : unescapeHtml($$_nRolePerson_Person.attr("name")));
    	...
    	arrRolePerson_person.push(($$_nRolePerson_Person.attr("ouname") == null) ? "" : unescapeHtml($$_nRolePerson_Person.attr("ouname")));
    	arrRolePerson_person.push(($$_nRolePerson_Person.attr("sipaddress") == null) ? "" : $$_nRolePerson_Person.attr("sipaddress"));
    	arrRolePerson_person.push(($$_nRolePerson_Person.attr("taskid") == null) ? "" : $$_nRolePerson_Person.attr("taskid"));
    	arrRolePerson_person.push(($$_nRolePerson_Person.attr("wiid") == null) ? "" : $$_nRolePerson_Person.attr("wiid"));
    	arrRolePerson_person.push(($$_nRolePerson_Person.attr("pfid") == null) ? "" : $$_nRolePerson_Person.attr("pfid"));
    	arrRolePerson_person.push(($$_nRolePerson_Person.attr("widescid") == null) ? "" : $$_nRolePerson_Person.attr("widescid"));
    }
...
function inserthtmlrow(objTB, index, viewtype, displayonly, taskType, arrDivision, arrDivisionTaskinfo, arrStep, arrStepTaskinfo, arrOu, arrOuTaskinfo, arrRolePerson, arrRolePersonTaskinfo, arrRolePersonTaskinfoComment, arrRolePersonTaskinfoCommentLast, arrRolePersonTaskinfoCommentFileInfo, description, arrRolePerson_person, displayBypass, arrRolePersonConsultation) {
    ...
    if (viewtype == "create" || viewtype == "change") {
        ...
        if (arrStep[2] == "parallel"
        	&& nTBodyRow[0] != null	
			&& $(nTBodyRow[0].childNodes[step_routetype]).text() == "receive"
            && nTBodyRow[nrowNo] != null	
            && ($(nTBodyRow[nrowNo].childNodes[step_unittype]).text() == "person" && arrStep[0] == "person"
            	|| $(nTBodyRow[nrowNo].childNodes[step_unittype]).text() == "ou" && arrStep[0] == "ou")
            && ($(nTBodyRow[nrowNo].childNodes[step_allottype]).text() == "parallel" )
        	&& ($(nTBodyRow[nrowNo].childNodes[step_routetype]).text() == arrStep[1] ) )//&& $(nTBodyRow[0].childNodes[step_routetype]).text() == "approve"
        {	
             var tmpIndexDot = tmpIndex.substring(0, tmpIndex.lastIndexOf("-") + 1).split("-").join(".");
	         tmpIndex = tmpIndex.substring(tmpIndex.lastIndexOf("-") + 1);
	         tmpIndex = dblDigit(Number(tmpIndex));
    ...
    var oCell_division_processID = oRow.insertCell(division_processID);
    var oCell_division_processDescID = oRow.insertCell(division_processDescID);
    var oCell_ou_taskid = oRow.insertCell(ou_taskid);
    var oCell_ou_wiid = oRow.insertCell(ou_wiid);
    var oCell_ou_pfid = oRow.insertCell(ou_pfid);
    var oCell_ou_widescid = oRow.insertCell(ou_widescid);     
    ...
    if (arrRolePerson != null && arrRolePerson[0] != null) {
        ...
        oCell_roleperson_person_taskid.innerHTML = (arrRolePerson[9] == null) ? "" : arrRolePerson[9];
        oCell_roleperson_person_wiid.innerHTML = (arrRolePerson[10] == null) ? "" : arrRolePerson[10];
        oCell_roleperson_person_pfid.innerHTML = (arrRolePerson[11] == null) ? "" : arrRolePerson[11];
        oCell_roleperson_person_widescid.innerHTML = (arrRolePerson[12] == null) ? "" : arrRolePerson[12];
    } else {
    ...
    if (arrRolePersonTaskinfo != null && arrRolePersonTaskinfo[0] != null) {
        ...
        if (taskType == "RolePerson") {
            if ($(oCell_roleperson_taskinfo_kind).text() == "skip") oCell_key_readonly.innerHTML = "True";
	...
	//if ((oCell_key_readonly.innerHTML == "True") && 
	    ...
	    if(arrStep.length != 0) {
        	...
        	if (oCell_division_taskinfo_kind.innerHTML != "receive") {
	        	...
	        	oRow.addEventListener('drop', function(event) { void_drop(event, id); });
...
function void_drop(pObjEvent, id) {
	...
	if(id == "tblApvLine"){
		...
		// 추가 전 Rows
		var nBeforeRowsCount = $("#tblccinfo > tbody > tr").length;
				
		// 추가
		document.getElementById("btSendCC").click();
				
		// 추가 후 Rows
		var nAfterRowsCount = $("#tblccinfo > tbody > tr").length;
				
		// 1건 추가되었으면 정상
		if (nBeforeRowsCount + 1 == nAfterRowsCount) {
			// 이동
			var nSelectIndex = 0;
			$("#tblccinfo > tbody > tr").each(function(pIntIndex) {
				if ($(this).attr("draggable")) {
					nSelectIndex = pIntIndex;
					return false;
				}
			...
			$("#tblccinfo > tbody > tr").eq(nSelectIndex).insertBefore($(oTR));
				 	
			$("#tblccinfo > tbody > tr").each(function () {
				if ($(this).attr("data-type") != null) {
					$(this).remove();
				}
			});
...
function void_dragOver(pObjEvent) {
	...
	if ((pObjEvent.offsetY > 0) &&
		...
		if ($(oTR).prev().attr("data-type") == null) {
			$(oTR).before("<tr id='hTRTemp' data-type='temp' ondrop=\"void_drop(event,'" + $(oTR).parent().parent()[0].id +  "')\" ondragover=\"event.preventDefault();\"><td colspan='" + $(oTR).find("td").length + "' style='background-color: #ffffe2;'></td></tr>");
		}
	}
		...
		if ($(oTR).next().attr("data-type") == null) {
			$(oTR).after("<tr id='hTRTemp' data-type='temp' ondrop=\"void_drop(event,'" + $(oTR).parent().parent()[0].id +  "')\" ondragover=\"event.preventDefault();\"><td colspan='" + $(oTR).find("td").length + "' style='background-color: #ffffe2;'></td></tr>");
		}
	}
	else {
		if ($(oTR).prev().attr("data-type") != null) {
...
function inserthtmlrowCC(objTB,taskType, arrCCinfo, arrOu, arrPerson) {
    ...
    if ($(oTable).find('tbody').length <= 0) {
		...
		oTBody =$("#tblccinfo").children("tbody")[0];
    }

    oTable.appendChild(oTBody);
    var nTBodyRow = oTBody.children;
    var nrowNo;
	if( nTBodyRow.length > 1){
		for(var i = 0; i < nTBodyRow.length; i++){
			if (nTBodyRow[i].getAttribute("id") == 'hTRTemp'){
				nrowNo = i+1;		
			}
		}
	}else{
		nrowNo = nTBodyRow.length;	
	}
    var oRow = oTBody.insertRow(nrowNo);
...
function statusChangeALL(e) {
    ...
    switch (acmpKey[3]) {
        ...
        case "normal": //일반결재
            ...
            oSelTR.childNodes[key_readonly].innerHTML = "False";
            changePrevInactive(oSelTR);
    ...
    while (oTempTR.previousSibling != null) {
        ...
        if (oTempTR.tagName == "TR") {
            if (oTempTR.childNodes[roleperson_taskinfo_kind].innerHTML == "skip") {
                ...
                oTempTR.childNodes[key_readonly].innerHTML = "False";
            	...
            	if (oTempTR.childNodes[division_taskinfo_kind].innerHTML != "receive") {
            		...
            		oTempTR.addEventListener('drop', function(event) { void_drop(event, id); });
...
function setDistDept($$_oList) {
    ...
    $$_elmList.concat().each(function (i, $$_emlNode) {
       if (chkDuplicate($$_emlNode.attr("AN"))) {
            var sDN = unescapeHtml($$_emlNode.attr("DN"));
    ...
    for (var i = 0; i < sRec.length; i++) {
        if (sRec[i] != "" && sRec[i] != null) {
            ...
            if(standalone_mode) {
                ...
                eTD.innerHTML = m_oFormEditor.getLngLabel(strName, false, "^"); //strName.split('^')[0];
                eTD.height = 20 + "px";
            } else {
...
var division_processID = 78 + CON_PRE_TD_COUNT; // div processID
var division_processDescID = 79 + CON_PRE_TD_COUNT; // div processDescID
var ou_taskid = 80 + CON_PRE_TD_COUNT; // ou taskid
var ou_wiid = 81 + CON_PRE_TD_COUNT; //ou  wiid
var ou_pfid = 82 + CON_PRE_TD_COUNT; //ou  pfid
var ou_widescid = 83 + CON_PRE_TD_COUNT; //ou widescid
var roleperson_person_taskid = 84 + CON_PRE_TD_COUNT; //person taskid
var roleperson_person_wiid = 85 + CON_PRE_TD_COUNT; //person wiid
var roleperson_person_pfid = 86 + CON_PRE_TD_COUNT; //person pfid
var roleperson_person_widescid = 87 + CON_PRE_TD_COUNT; //person widescid
