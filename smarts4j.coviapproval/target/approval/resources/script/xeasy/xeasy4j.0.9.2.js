	...
	*/
	...
	var tests = {
		...
		alphanumeric: function (a) {// a --> value of input
			...
			var regex = /^[a-z0-9]+$/i;
			try {
				if (regex && !new RegExp(regex).test(a))
					return false;
			}
			catch (err) {
				return false;
		...
		numeric: function (el) {
			var a = el.val();
			var regex = /^[0-9]+$/i;
			var n = (regex && !new RegExp(regex).test(a));
	...
	(function ($) { // alias the $ function for use with jquery in no-conflict mode
		$.fn.datePicker = function (json_option) {
			...
			$el.each(function () {
				...
				var yearRangeOpt = $(this).attr('date-year-range');
				if(!yearRangeOpt || yearRangeOpt == undefined){
					yearRangeOpt = "c-10:c+10";
				}
...
//150902: 변경: 부분합(열 소계) 기능 추가
	...
	var makeid = function () {
		...
		for (var i = 0; i < 5; i++)
			text += possible.charAt(Math.floor(coviCmn.random() * possible.length));
