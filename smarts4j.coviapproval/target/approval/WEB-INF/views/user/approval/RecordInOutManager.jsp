...
<script>	
			 	...
			 	{label: ' ', width: '15', align: 'center', sort: false,
			 		formatter: function(){
			 			...
			 			if(item.ListAuthorityName != "" && item.ListAuthorityID != ""){
			 				returnObj = $("<a>", {"class": "btnTypeDefault modifyBtn", "text": "<spring:message code='Cache.btn_Edit' />", "onclick": "openManagerPopup('M', '" + item.DeptCode  + "', '" +  item.DeptName + "')"}).get(0).outerHTML; // 수정
			 			}else{
	...
	var govDocFunction = {		 	
		init: function(){
			$("#refresh").on("click", function(){
				...
				ListGrid.bindGrid({ajaxUrl: "/approval/user/getGovRecordDocInOutManager.do", ajaxPars: {"searchType": $("#searchType").val()}});	
			});
	...
	function openManagerPopup(mode, deptCode, deptName){
		var popupName = "";
		var url = "/approval/user/govRecordDocInOutUserAdd.do?mode=" + mode;
		
		switch(mode){
			case "I":
				popupName = "<spring:message code='Cache.lbl_addRecordDocumentRecipient' />"; // 기록물철 문서관리자 추가
				break;
			case "M":
				popupName = "<spring:message code='Cache.lbl_modifyRecordDocumentRecipient' />"; // 기록물철 문서관리자 수정
				break;
		}
