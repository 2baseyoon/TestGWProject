					...
					<tbody>
						<tr>
							...
							<td>
								<div class="box">
									<span>
										<input id="selDept" type="input" readonly="readonly" value="${list[0].DeptName}" name="${list[0].DeptCode}">
...
<script>
	...
	$(document).ready(function(){
		...
		$("#btnSave").on("click", function(){
			var data = input.get();
			var deptcode = $("#selDept").attr('name');
			...
			$.ajax({
				url: "/approval/user/insertGovRecordDocInOutUser.do",
				type: "POST",
				data: {
		...
		$("#btnDelete").on("click", function(){
			Common.Confirm("<spring:message code='Cache.msg_RUDelete' />", "<spring:message code='Cache.lbl_apv_delete' />", function(result){
				if(result){
					...
					var deptcode = $("#selDept").attr('name');
					$.ajax({
						url: "/approval/user/deleteGovRecordDocInOutUser.do",
						type: "POST",
						data: {
							data: JSON.stringify(data)
							,deptcode : deptcode
		...
		if(CFN_GetQueryString("mode") == "M"){
			...
			$.ajax({
				url: "/approval/user/getGovRecordDocInOutManager.do",
				type: "POST",
				data: {
