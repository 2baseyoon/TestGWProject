...
Date.prototype.format = function(f) {
    ...
    var weekName = [Common.getDic("lbl_Sunday"), Common.getDic("lbl_Monday"), Common.getDic("lbl_Tuesday")
...
function clickHolidayLabel(thisObj){
	var idx = $(".chkHoliday").index(thisObj);
	
	if($("tr[class*=multi-row]").eq(idx).find(".chkHoliday").prop("checked")){
		...
		if(document.getElementsByName("_MULTI_SCHEDULE_SDT").length <= 1){
			...
			$("#tblWorkScheduleInfo").find(".multi-row").each(function(idx,item){
				if($(item).find("[name='WORK_TYPE']").val() == "N"
					...
					alert("직접입력을 선택한 경우, 근무일정 시간을 입력해야 합니다.");
					chkValid = true;
					return false;
				}
			});
...
function chkCompanyHoliday(idx) {
	...
	if(sDate != "" && eDate != ""){
		...
		$(".chkHoliday").eq(idx).closest("td").find("input[name=companyHoliday]").val(companyHoliday);
	}
}
...
function setSubject(){
	...
	if(!isNaN(date)) $("#approval_write_Subject").val(sHeadName+"_"+sUserName+"("+date.format('MM/dd')+")");
	else $("#approval_write_Subject").val(sHeadName+"_"+sUserName);
...
function displayTime(obj) {
	var tmpObj = $(obj).closest("tr[class*=multi-row]");
